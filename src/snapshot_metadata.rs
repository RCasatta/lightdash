...
fn summary_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("roic.periods[].annualized_gross_roic_percent".into(), aggregation(warning(formula(field("number", false, Some("percent"), "Annualized gross node return for the period using current local channel funds as deployed capital."), "period_forwarding_fees_sat * (12 / months) / channel_funds_sat * 100"), "The denominator is the current local balance, not historical capital deployed during the period."), "Do not average this value with channel-level capacity-return values.")),
...
fn channel_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("gross_capacity_return_percent".into(), aggregation(formula(field("number", true, Some("percent"), "Annualized lifetime forwarding return relative to full channel capacity."), "forwarding_fees_sat * 1000 / capacity_msat * 365 / age_days * 100"), "Use capacity-weighting rather than a simple average when combining channel returns.")),
        ("net_capacity_return_percent".into(), aggregation(formula(field("number", true, Some("percent"), "Annualized lifetime capacity return after subtracting target-attributed rebalance cost."), "net_routing_revenue_msat / capacity_msat * 365 / age_days * 100"), "Use capacity-weighting rather than a simple average when combining channel returns.")),
