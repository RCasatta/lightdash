...
fn summary_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("roic.periods[].annualized_gross_roic_percent".into(), aggregation(warning(formula(field("number", false, Some("percent"), "Annualized gross node return from forwarding and earned lease fees, using current local channel funds as deployed capital."), "(period_forwarding_fees_sat * 1000 + period_lease_fee_earnings_msat) * (12 / months) / (channel_funds_sat * 1000) * 100"), "The denominator is the current local balance, not historical capital deployed during the period."), "Do not average this value with channel-level capacity-return values.")),
        ...
        ("roic.net_roic_12_months_percent".into(), formula(field("number", false, Some("percent"), "Annualized trailing net return from forwarding and lease fees after subtracting lease-fee and rebalance costs."), "(forwarding_fees_12_months_sat * 1000 + lease_fee_earnings_12_months_msat - lease_fee_cost_12_months_msat - rebalance_cost_12_months_msat) / (channel_funds_sat * 1000) * 100")),
    ])
}
