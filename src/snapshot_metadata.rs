...
fn summary_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("pending_channel_balance_msat".into(), warning(formula(field("integer", false, Some("msat"), "Estimated local balance in non-normal channels whose funds have not fully resolved into wallet outputs."), "sum(listfunds.channels[state != CHANNELD_NORMAL and listpeerchannels.status does not contain 'All outputs resolved'].our_amount_msat)"), "This is an estimate based on the latest channel balance. Final on-chain fees and unresolved HTLC outcomes can slightly change the amount returned to the wallet.")),
        ("pending_channel_count".into(), formula(field("integer", false, Some("channel"), "Number of non-normal channels included in pending_channel_balance_msat."), "count(listfunds.channels[state != CHANNELD_NORMAL and listpeerchannels.status does not contain 'All outputs resolved'])")),
        ("estimated_total_balance_msat".into(), warning(formula(field("integer", false, Some("msat"), "Estimated total funds controlled by the node across normal channels, wallet outputs, and unresolved channel claims."), "sum(listfunds.channels[state = CHANNELD_NORMAL].our_amount_msat) + onchain_balance_msat + pending_channel_balance_msat"), "Pending channel claims can change slightly because of final on-chain fees and unresolved HTLC outcomes.")),
...
fn channel_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("private".into(), source(warning(field("boolean", true, None, "Whether the local channel is configured not to be announced to the gossip network."), "Null means listpeerchannels did not report the private flag for this channel."), "listpeerchannels.channels.private joined by channel_id")),
        ...
        ("outbound_fee_ppm".into(), source(field("integer", true, Some("ppm"), "Current proportional fee configured by the local node for outbound forwarding."), "listpeerchannels local update (or legacy top-level fee), falling back to the listchannels direction sourced by the local node")),
        ("inbound_fee_ppm".into(), source(field("integer", true, Some("ppm"), "Current proportional fee advertised by the remote peer toward the local node."), "listpeerchannels remote update, falling back to the listchannels direction sourced by the peer")),
        ("outbound_base_fee_msat".into(), source(field("integer", true, Some("msat"), "Current fixed fee configured by the local node for outbound forwarding."), "listpeerchannels local update (or legacy top-level fee), falling back to the listchannels direction sourced by the local node")),
        ("outbound_htlc_min_msat".into(), source(field("integer", true, Some("msat"), "Current minimum HTLC accepted by the local outbound policy."), "listpeerchannels local update, falling back to the listchannels direction sourced by the local node")),
        ("outbound_htlc_max_msat".into(), source(field("integer", true, Some("msat"), "Current maximum HTLC accepted by the local outbound policy."), "listpeerchannels local update, falling back to the listchannels direction sourced by the local node")),
        ("outbound_delay_blocks".into(), source(field("integer", true, Some("block"), "Current CLTV delta required by the local outbound policy."), "listpeerchannels local update, falling back to the listchannels direction sourced by the local node")),
        ...
        ("time_decayed_fee_ppm".into(), warning(formula(field("number", true, Some("ppm"), "Amount-weighted full outbound fee rate with recent forwards weighted more heavily."), "sum(fee_msat * 0.5^(age_seconds / 604800)) * 1000000 / sum(out_msat * 0.5^(age_seconds / 604800)) for settled outbound forwards with out_msat >= 1000000"), "Uses a seven-day half-life, includes the base fee, and excludes outbound forward parts smaller than 1,000 sats.")),
        ...
        ("rebalance_source_debit_msat".into(), warning(field("integer", false, Some("msat"), "All matched rebalance debits funded by this channel when it was the source."), "Includes both liquidity delivered to target channels and the rebalance fees paid.")),
        ("rebalance_source_debit_90d_msat".into(), warning(formula(field("integer", false, Some("msat"), "Matched rebalance debits funded by this channel during the 90 days ending at snapshot generation."), "sum(rebalance source debit_msat where snapshot_generated_at - 90 days <= resolved_at <= snapshot_generated_at)"), "Includes both liquidity delivered to target channels and the rebalance fees paid; parts without a timestamp are excluded.")),
        ("rebalance_source_effective_fee_ppm".into(), formula(field("number", true, Some("ppm"), "All-time effective route cost of rebalances funded by this channel."), "rebalance_source_cost_msat * 1000000 / sum(rebalance source credit_msat)")),
        ...
        ("combined_capacity_return_percent".into(), warning(aggregation(formula(field("number", true, Some("percent"), "Annualized channel contribution combining direct net return and incoming-side indirect attribution."), "net_capacity_return_percent + indirect_capacity_contribution_percent"), "Use capacity-weighting rather than a simple average when comparing groups of channels."), "This is a per-channel contribution metric. Do not sum it across the node as unique revenue because indirect attribution duplicates revenue earned on outgoing channels.")),
    ])
}
...
fn closed_channel_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("combined_capacity_return_percent".into(), warning(formula(field("number", true, Some("percent"), "Annualized closed-channel contribution combining direct net return and incoming-side indirect attribution."), "net_capacity_return_percent + indirect_capacity_contribution_percent"), "Do not aggregate as additional node revenue because indirect attribution duplicates revenue earned on outgoing channels.")),
    ])
}
