...
fn summary_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("channel_funds_sat".into(), warning(formula(field("integer", false, Some("sat"), "Current local balance held in CHANNELD_NORMAL channels; used as the node-level deployed-capital denominator."), "sum(normal_channel.local_balance_msat) / 1000"), "This is local balance, not the sum of full channel capacities.")),
        ...
        ("roic.periods[].annualized_gross_roic_percent".into(), aggregation(warning(formula(field("number", false, Some("percent"), "Annualized gross node return for the period using current local channel funds as deployed capital."), "period_forwarding_fees_sat * (12 / months) / channel_funds_sat * 100"), "The denominator is the current local balance, not historical capital deployed during the period."), "Do not average this value with channel-level ROIC values.")),
        ...
        ("roic.capital_velocity_12_months".into(), formula(field("number", false, Some("ratio"), "Trailing routed volume divided by current local channel funds."), "routed_12_months_sat / channel_funds_sat")),
        ("roic.effective_fee_rate_12_months_bps".into(), formula(field("number", false, Some("bps"), "Fees earned per routed amount over the trailing 360 days."), "forwarding_fees_12_months_sat * 10000 / routed_12_months_sat")),
        ...
        ("roic.net_roic_12_months_percent".into(), formula(field("number", false, Some("percent"), "Annualized trailing net return after subtracting rebalance costs."), "(forwarding_fees_12_months_sat * 1000 - rebalance_cost_12_months_msat) / (channel_funds_sat * 1000) * 100")),
    ])
}
...
fn channel_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("gross_roic_percent".into(), aggregation(formula(field("number", true, Some("percent"), "Annualized lifetime forwarding return relative to full channel capacity."), "forwarding_fees_sat * 1000 / capacity_msat * 365 / age_days * 100"), "Use capacity-weighting rather than a simple average when combining channel returns.")),
        ("net_roic_percent".into(), aggregation(formula(field("number", true, Some("percent"), "Annualized lifetime return after subtracting target-attributed rebalance cost."), "net_routing_revenue_msat / capacity_msat * 365 / age_days * 100"), "Use capacity-weighting rather than a simple average when combining channel returns.")),
        ("indirect_roic_percent".into(), warning(aggregation(formula(field("number", true, Some("percent"), "Annualized incoming-side fee attribution relative to full channel capacity."), "indirect_fees_sat * 1000 / capacity_msat * 365 / age_days * 100"), "Do not aggregate across the node as revenue."), "Revenue is earned on the paired outgoing channel; this metric describes the incoming channel's contribution.")),
    ])
}
...
fn closed_channel_fields() -> BTreeMap<String, FieldMetadata> {
    BTreeMap::from([
        ...
        ("net_roic_percent".into(), formula(field("number", true, Some("percent"), "Annualized lifetime forwarding return after target-attributed rebalance costs."), "net_routing_revenue_msat / capacity_msat * 365 / age_days * 100")),
        ("indirect_roic_percent".into(), warning(field("number", true, Some("percent"), "Annualized incoming-side fee attribution over the closed channel lifetime."), "Do not aggregate as additional node revenue.")),
    ])
}
