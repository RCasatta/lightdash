...
fn remove_stale_correlations_page(directory: &str) {
    let file_path = format!("{directory}/correlations.html");
    match fs::remove_file(&file_path) {
        Ok(_) => log::info!("Removed stale correlations page: {file_path}"),
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {}
        Err(e) => log::error!("Error removing stale correlations page {file_path}: {e}"),
    }
}
...
fn create_channel_pages(input: ChannelPagesInput<'_>) {
    ...
    let balance_target_stddev = channel_balance_target_stddev_percentage_points(channels);
    let balance_target_variance = balance_target_stddev * balance_target_stddev;
...
fn create_rebalance_page(directory: &str, store: &Store, now: &chrono::DateTime<chrono::Utc>) {
    ...
    let rebalance_html = wrap_in_html_page(
        "Rebalance Recap",
        rebalance_content,
        &now.format("%Y-%m-%d %H:%M:%S UTC").to_string(),
    );

    let rebalance_file_path = format!("{}/rebalance.html", directory);
    match fs::write(&rebalance_file_path, rebalance_html.into_string()) {
        Ok(_) => log::info!("Rebalance page generated: {}", rebalance_file_path),
        Err(e) => log::error!("Error writing rebalance page: {}", e),
    }
}
...
pub fn run_dashboard(
    ...
    remove_stale_correlations_page(&directory);
    log::info!(
